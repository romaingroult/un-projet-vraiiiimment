    ...
    void makeCredentials(uint8_t *command, uint8_t length){
        if(length != 1 + APP_ID_SIZE){
            UART__putc(STATUS_ERR_BAD_PARAMETER);
            return;
        }
        uint8_t app_id[APP_ID_SIZE];
        memcpy(app_id, &command[1], APP_ID_SIZE);
    ...
    void getAssertion(uint8_t *command, uint8_t length){
        if(length != 1 + APP_ID_SIZE + CLIENT_DATA_SIZE){
            UART__putc(STATUS_ERR_BAD_PARAMETER);
            return;
        }
        ...
        memcpy(app_id, &command[1], APP_ID_SIZE);
        memcpy(client_data, &command[1+APP_ID_SIZE], CLIENT_DATA_SIZE);
